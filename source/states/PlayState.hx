...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Uninstall the game', 0.2], // From 0% to 19%
		...
		['TIMELESS!!!', 1], // From 90% to 99%
		['TIMELESS!!!', 1] // The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var glitchNotesModifier:Float = 0.0;
	...
	override public function create()
	{
		...
		crashOnMiss = ClientPrefs.getGameplaySetting('crashOnMiss');
		healthDrain = ClientPrefs.getGameplaySetting('healthDrain');
		glitchNotesModifier = ClientPrefs.getGameplaySetting('glitchNotesModifier');
	...
	public dynamic function updateScoreText()
	{
		...
		if (scoreTxt != null && scoreTxt.text != tempScore) {
	...
	private function generateSong():Void
	{
		...
		try {
				if (songData.events != null) {
						for (event in songData.events) {
								if (event != null) {
										var subEvents:Array<Dynamic> = cast event;
										for (subEvent in subEvents) {
												if (subEvent != null && (subEvent == 'Change Character' || subEvent == 'ChangeCharacter')) {
														var targetChar:String = Std.string(subEvent[1]);
														var targetType:String = Std.string(subEvent[2]);
								
														var charGroup:Int = 1; 
														if (targetType == '0' || targetType == 'dad') charGroup = 0;
														if (targetType == '2' || targetType == 'gf') charGroup = 2;
								
														if (targetChar != null && targetChar.trim() != '') {
																addCharacterToList(targetChar, charGroup);
														}
												}
										}
								}
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				if (glitchNotesModifier > 0 && swagNote.mustPress && FlxG.random.bool(glitchNotesModifier * 100)) { 
					swagNote.noteType = 'Glitch Note';  
				}
...
{
	if (camHUD != null) camHUD.visible = (notes.length > 0 || !generatedMusic);
	if (camGame != null) camGame.visible = (generatedMusic && !isDead);
...
{
		if (healthDrain && !practiceMode && !cpuControlled) {
		health -= 0.023;
...
public function goodNoteHit(note:Note):Void
{
	...
	if (!note.hitCausesMiss) // Common notes
	{
		...
		if (sickOnly && note.rating != 'sick' && !note.isSustainNote) {
